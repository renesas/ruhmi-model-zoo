...
__attribute__((aligned(16))) uint8_t sub_0000_arena[245760];
// Fast scratch arena not used for Ethos-U55
//  We will not create it for now and reuse the address of the other arena
// __attribute__((aligned(16))) static uint8_t sub_0000_fast_scratch[245760];
...
int sub_0000_invoke(bool clean_outputs) {
  ...
  // Buffer sub_0000_model with size 473296 and address: 4294967295
  base_addrs[0] = (uint64_t)(uintptr_t)sub_0000_model_data;
  base_addrs_size[0] = sub_0000_model_data_size;
  // Buffer sub_0000_arena with size 245760 and address: 0
  base_addrs[1] = (uint64_t)(uintptr_t) (sub_0000_arena+0);
  base_addrs_size[1] = 245760;

  // Buffer sub_0000_fast_scratch with size 245760 and address: 0
  base_addrs[2] = (uint64_t)(uintptr_t) (sub_0000_arena+0);
  base_addrs_size[2] = 245760;

  // Buffer input_tensor_0 with size 1024 and address: 196608
  base_addrs[3] = (uint64_t)(uintptr_t) (sub_0000_arena+196608);
  base_addrs_size[3] = 1024;

  // Buffer output_tensor_0 with size 8 and address: 0
  if (clean_outputs) {
    memset(sub_0000_arena + 0, 0, 8);
  }
