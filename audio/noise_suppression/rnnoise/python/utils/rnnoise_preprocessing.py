...
class RNNoisePreProcess:
    ...
    def _compute_frame_features(self, audio_window):
        ...
        if (not self.training) and E < 0.04:
            ...
            # RUHMI fix 2026-06-10: upstream Arm ML-zoo returned 2 values here while the
            # non-silence path and the caller `process_frame` both unpack 7. By this point
            # X, P, Ex, Ep, Exp are all computed, so we just include them. Without this,
            # any frame with band-energy < 0.04 (e.g. silence, low-amplitude inputs) raises
            # "ValueError: not enough values to unpack (expected 7, got 2)".
            return True, features, X, P, Ex, Ep, Exp
