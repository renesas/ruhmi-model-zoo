# MobileNetV1 (alpha=0.25) — Image Classification (ImageNet)

MobileNetV1 is a lightweight depthwise-separable convolutional network ([Howard et al., 2017](https://arxiv.org/abs/1704.04861)) designed for mobile and embedded vision applications. This folder contains everything needed to obtain the model, convert it to TFLite, compile it for the Renesas RA8P1 (Cortex-M85 + Ethos-U55 NPU) using the RUHMI toolchain, and run inference.
...
| Field | Value |
|-------|-------|
| **Model name** | MobileNetV1 (alpha=0.25) |
...
|---------------|--------|:---------:|:---------:|
| TFLite FP32 | `.tflite` | 48.32 | 72.78 |
| TFLite INT8 | `.tflite` | 46.00 | 65.97 |
| MERA FP32 | `.mera` | 48.32 | 72.78 |
| MERA INT8 (TFLite Quantized) | `.mera` | 45.95 | 70.66 |
...
|----------------------|:--------:|:--------:|
| † Internal Flash + Internal SRAM | 42 | 1 |
| ¶ Internal SRAM only | 43 | 1 |
...
2. **Inference venv** — navigate to the `python/` directory and create a dedicated virtual environment:
    ...
    **Ubuntu / bash**
    ...
    ```bash
    cd vision/image_classification/mobilenetv1/python
    python3.10 -m venv .venv_mobilenetv1
    source .venv_mobilenetv1/bin/activate
    pip install --upgrade pip
    pip install -r requirements.txt
    ```
...
|------|---------|-------------|
| `--model` | `model/mobilenet_v1_FP32.tflite` | Path to TFLite model |
...
Confidence (score) : 0.6831
========================================
```

> By default `inference.py` loads `model/mobilenet_v1_FP32.tflite`. Pass `--model model/mobilenet_v1_INT8.tflite` to use the INT8 variant.
...
```yaml
model_path: vision/image_classification/mobilenetv1/python/model/mobilenet_v1_INT8.tflite
output_dir: vision/image_classification/mobilenetv1/embedded_c/src_mcu
...
MobileNetV1 weights are provided by the TensorFlow / Keras team under the [Apache License 2.0](http://www.apache.org/licenses/LICENSE-2.0) via `tf.keras.applications`.
