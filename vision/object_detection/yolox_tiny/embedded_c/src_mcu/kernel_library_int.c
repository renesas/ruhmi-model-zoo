...
/*
 * Shared scratch buffer for kernel functions that need a temporary float array.
 * Original code used VLAs on the stack (e.g. float dq_buffer[size]) which causes
 * stack overflow when size is large (up to 301,056 → 1.2 MB).
 * This static buffer is placed in SDRAM and reused by Sigmoid, Tanh, Exp, Log,
 * Relu6, etc. — they are never called concurrently so sharing is safe.
 * Max size: 301,056 floats = 1,204,224 bytes (~1.15 MB).
 */
#define KERNEL_SCRATCH_MAX_FLOATS (301056U)
static float g_kernel_scratch[KERNEL_SCRATCH_MAX_FLOATS]
    __attribute__((aligned(16), section(".sdram_noinit")));
...
void Sigmoid(const int8_t *input, int8_t *output, uint32_t size,
  ...
  float *dq_buffer = g_kernel_scratch;
...
void Tanh(const int8_t* input, int8_t* output, uint32_t size, float input_scale, int8_t input_zp,
  ...
  float *transit_out = g_kernel_scratch;
  float working_val, floor_val, diff;
